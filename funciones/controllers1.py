...
def getData(rut):
    ...
    for periodo in periodos:
        ...
        try:
            ...
            cantidad = 0
            for row in rows:
                #hay que borrar todos los print que se encuentren en el archivo principal
                tipo = row.find_element(By.CSS_SELECTOR,'td[scope="row"]').text
                cols = row.find_elements(By.CSS_SELECTOR,'td.ng-binding')
                col = cols[0].text
                valor = int(col)
...
def data_clientes():
    ...
    try:
        ...
        print (e)
        return {"error": "Error en el proceso de obtención de datos de facturación"}


def datos_del_cliente ():
    datos = request.json
    rut = datos["rut"]
    password = datos["password"]
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                #abrir seccion de actividad economica
                actividad_economica_btn = driver.find_element(By.ID,'ctracc_2')
                actividad_economica_btn.click()

                time.sleep(3)
                telefono = driver.find_element(By.XPATH,'/html/body/div[1]/div[1]/div[5]/div[2]/div[3]/div[2]/div/div[2]/div[3]/div/table/tbody/tr[2]/td[2]').text
                #print(telefono)
